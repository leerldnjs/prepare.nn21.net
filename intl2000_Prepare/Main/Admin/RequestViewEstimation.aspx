<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RequestViewEstimation.aspx.cs" Inherits="Admin_RequestViewEstimation" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
    	...
    	function PopupItemModifyList(pk) { //수정내역 보기
    		window.open('../Request/ItemModifyList.aspx?S=' + pk + '&CL=71', '', 'location=no, directories=no, resizable=no, status=no, toolbar=no, menubar=no, scrollbars=no, top=200px; left=200px; height=700px; width=600px;');
    	}
    	...
    	function RequestWriteforEstimation() {
    		if (confirm("이 내용으로 새로운 접수증을 등록 하시겠습니까? 운임부분은 새로 등록 하셔야 됩니다")) {
    			Admin.RequestWriteforEstimation(form1.HRequestFormPk.value, form1.HAccountID.value, function (result) {
    				if (result == "1") {
    					alert("SUCCESS");
    					location.reload();
    				}
    	...
    	function SetRequestFormStep(how) {
    		...
    		switch (how) {
    			...
    			case 'FixCharge':
    				...
    				var retVal = window.showModalDialog('./Dialog/FixCharge.aspx?S=' + form1.HRequestFormPk.value, dialogArgument, "dialogHeight:600px; dialogWidth:660px; resizable:0; status:0; scroll:1; help:0; ");
    				if (retVal == "Y") { window.document.location.reload(); }
    				break;
    			case "ModifyCharge":
    			...
    			case "deferRestore":
    				...
    				break;
    			case "ShipperCharge":
    				window.open('../Request/FreightChargeView.aspx?S=' + form1.HRequestFormPk.value + "&G=S", '', 'location=no, directories=no, resizable=no, status=no, toolbar=no, menubar=no, scrollbars=yes, top=200px; left=200px; height=700px; width=800px;');
    				break;
    			case "ConsigneeCharge":
    				window.open('../Request/FreightChargeView.aspx?S=' + form1.HRequestFormPk.value + "&G=C", '', 'location=no, directories=no, resizable=no, status=no, toolbar=no, menubar=no, scrollbars=yes, top=200px; left=200px; height=700px; width=800px;');
    				break;
    	...
    	function FileUpload(requestFormPk) {
    		window.open('./Dialog/FileUpload.aspx?G=2&S=' + requestFormPk, '', 'location=no, directories=no, resizable=no, status=no, toolbar=1, menubar=no, scrollbars=no, top=200px; left=200px; height=	300px; width=600px;');
    	}
        ...
        <div style="background-color: White; width: 1050px; height: 100%; padding: 25px;">
            <div style="float: right; width: 590px;">
                ...
                <fieldset>
                    ...
                    <input type="button" value="OK" onclick="ContentsAdd('1');" />
                </fieldset>
                <%=HtmlMemo %>
            ...
            <div style="width: 450px;">
                ...
                <div><span id="DVMakeBL"></span><%=HtmlButton %><br />
                    <%=HtmlButtonPayment %> </div>
