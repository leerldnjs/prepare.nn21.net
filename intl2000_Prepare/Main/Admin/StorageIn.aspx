<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StorageIn.aspx.cs" Inherits="Admin_StorageIn" Debug="true" %>
...
<head id="Head1" runat="server">
	...
	<script type="text/javascript">
			...
			Admin.PrepareTransportOnloadOurStorage(form1.HBranchPk.value, function (result) {
				...
				for (var q = 0; q < result.length; q++) {
					...
					if (Each[6] == "") {
						...
						"</td>" +
						"<td " + IsDefer + "><a href='./RequestView.aspx?g=s&pk=" + Each[0] + "'>" + Each[1] + " ~ " + Each[2] + "</a></td>";
					}
					else {
						OurStorage += "<tr>" +
					...
					OurStorage += "<td " + IsDefer + ">" + Each[11] + "</td>" +
						...
						"<td " + IsDefer + ">" + Each[5] + "</td></tr><tr><td class=\"TBody1\">" +
						...
						"</td><td colspan='7' class=\"TBody1\"><input type=\"text\" id=\"TBRequestComment[" + q + "]\"  onblur=\"BTNAddReqeustComment('" + q + "');\" style='width:600px; font-style:italic; border:0px; ' value=\"" + Each[14] + "\" /></td></tr>";
				}
				if (NowStorage != "") {
					OurStorage += "<tr><td colspan='8' style=\"background-color:#bbbbbb;\">Total Box : " + TotalBoxCount + " &nbsp;&nbsp;&nbsp;Total Weight : " + parseInt(TotalWeight + 0.00000001) / 100000 + "&nbsp;&nbsp;&nbsp;Total Volume : " + parseInt(TotalVolume + 0.00000001) / 100000 + "</td></tr>";
				}
			...
			Admin.PrepareTransportOnloadHead(form1.HBranchPk.value, function (result) {
				...
				for (var i = 0; i < result.length; i++) {
					...
					switch (each[0]) {
						...
						case "2":
							...
							Html += "		<input type=\"radio\" name=\"TransportBBHead\" id=\"TransportBBHead[" + each[1] + "]\" value=\"" + each[1] + "\" /><label for=\"TransportBBHead[" + each[1] + "]\"><strong> 차량 => " + each[7] + "</strong>" +
										...
										"			<div style=\"border-bottom:1px dotted #cccccc;\">" + each[3] + " ~ " + each[4] + "</div>" +
										"			<div style=\"border-bottom:1px dotted #cccccc;\">" + description[2] + " ~ " + description[3] + "</div>" +
										"			<div style=\"border-bottom:1px dotted #cccccc;\">" + HTMLFILE + "</div>" +
										"		</div></label>";
							break;
						case "3":
					...
					Html += "		<div style=\"width:300px; border:1px dotted #FFFFFF; padding:5px;  \"  >" + HTMLCOMMENT +
									...
									"		</div>" +
									"		<div style='text-align:right;'><input type=\"button\" value=\"File Upload\" onclick=\"GoFileupload('" + each[1] + "');\" /><input type=\"button\" value=\"Modify\" onclick=\"PackingModify('" + each[1] + "', '" + each[0] + "');\" /><input type=\"button\" value=\"Cancel\" onclick=\"PackingCancle('" + each[1] + "');\" />" + btnSend + "</div>" +
										Summary + "	</div>";
		...
		function SelectTransportWay(gubun) {
			if (gubun == "T") {
				...
				window.open('./Dialog/SelectTransportWay.aspx', '', 'location=no, directories=no, resizable=no, status=no, toolbar=1, menubar=no, scrollbars=no, top=200px; left=200px; height=700px; width=600px;');
			}
		}
	...
	<style type="text/css">
		...
		div#TransportBB{ }
		div#TransportEach{padding:10px; width:320px; float:left; margin-left:10px; margin-right:10px; }
		div#quick_menu {border:1px solid #999999; background:#eeeeee; clear:both; text-align:center; }
	</style>
...
<body style="background-color:#E4E4E4; width:900px; margin:0 auto; padding-top:10px;" >
	...
	<div style="background-color:White; width:850px; height:100%; padding:25px;">
		...
		<div id="TransportBB"><%=PrepareTransport.ToString() %></div>
		<div id="quick_menu">
			<div>
				...
				<input type="button" value="+ <%=GetGlobalResourceObject("qjsdur", "dnsthddPdircnrk") %> " onclick="SelectTransportWay('');" style="width:100px;" />
				<input type="button" value="T" onclick="SelectTransportWay('T');" style="width:15px;" />
